...
mo_chit,saphan_kwai,ari,sanam_pao,victory_monument,phaya_thai,ratchathewi,siam,chit_lom,ploen_chit,nana,asok,
phrom_phong,thong_lo,ekkamai,phra_khanong,on_nut,bang_chak,punnawithi,udom_suk,bang_na,bearing,samrong,national_stadium,
siam_silom_line,ratchadamri,sala_daeng,chong_nonsi,surasak,saphan_taksin,krung_thon_buri,wongwian_yai,pho_nimit,
talat_phlu,wutthakat,bang_wa,hua_lamphong,sam_yan,si_lom,lumphini,klong_toei,queen_sirkit_national_covention_center,
sukhumvit,phetchaburi,phra_ram_9,thailand_cultural_center,huai_kwang,sutthisan,ratchadaphisek,lat_phrao,phahon_yothin,
chatuchak_park,kamphaen_phet,bang_sue,tao_poon,khlong_bang_phai,talad_bang_yai,sam_yaek_bang_yai,bang_phlu,bang_rak_yai,
bang_rak_noi_tha_it,sai_ma,phra_nang_klao_bridge,yaek_nonthaburi1,bang_krasor,nonthaburi_civic_center,
ministry_of_public_health,yaek_tiwanon,wong_sawang,bang_son,tao_poon_purple]).
%=======================================================================================================================
...
astar(Start, Destination, Path, Cost) :-
    getHn(Start,Destination,Hn),
    search(Destination,[[Start,[Start],Hn]],[_,Path,Cost]).

search(Station, [[Station,Path,TotalCost]|T], [Station,Path,TotalCost]) :- !.
search(Destination, [[Station,Path,TotalCost]|T], Result) :-
        expand([Station,Path,TotalCost],Destination,ExpandedNode),
...
expand([Station,Path,_], Destination, Return) :-
        findall(X, distance(Station,X,_),NextStations),
...
createNode([Station|T], Destination,Path,Nodes, ExpandedNodes) :-
        append(Path, [Station], NewPath),
...
getGn([], From, Acc, Acc).
getGn([To|T], From, Acc, Gn) :-
      distance(From,To,StepCost), !, NewAcc is Acc + StepCost, getGn(T, To, NewAcc, Gn).

power(X,Result) :- Result is X * X.
getHn(Start, Destination, HeuristicValue) :-
      location(_,Start,X1,Y1), location(_,Destination,X2,Y2),
      generateDistance(X1,Y1,X2,Y2,Distance),
      HeuristicValue is Distance/106.666667. %max deltaV train

generateDistance(X1, Y1, X2, Y2, ReturnV2) :-
      R is 6371e3, % metres
...
minsort(List, Return) :-
     setof([TotalCost,Station,Cost], member([Station,Cost,TotalCost], List), Result),
...
% Which Train
%=======================================================================================================================
trainPath([Station|Path], Col, Row, Result) :-
    location(Train, Station, _, _), whichTrain(Path, [Train], TrainList),
    includeWalk(TrainList, Row, Col, Result). %confusion between width and row = ="
				     

whichTrain([], TrainList, TrainList).
whichTrain([Station|Path], TrainList, Result) :-
     location(Train, Station, _, _),
...
checkWalk([H|T], Result, AccR, AccC, Row, Col, Return) :-
    isDifferent(H,T) -> append(Result, [walk], X), NewAccC is AccC + 1,
                        checkWalk(T, X, AccR, NewAccC, Row, Col, Return)
          ;append(Result, [H],X), NewAccR is AccR + 1,
          checkWalk(T, X, NewAccR, AccC, Row, Col, Return).
...
%=======================================================================================================================
findClosestStation(X1, Y1, Result) :-
    stationList(Stations), findMinDistance(Stations, X1, Y1, [9999,a], Result).

findMinDistance([], X1, Y1, [Distance|Station], [Station,Distance]).
findMinDistance([HStation|TStation], X1, Y1, [Distance|Station], Result) :-
    location(_,HStation,X2,Y2),
